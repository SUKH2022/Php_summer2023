...
if (isset($_POST['submit'])) {
    ...
    if (!$title) {
        $_SESSION['edit-post'] = "Couldn't update the post. Invalid form data.";
    } elseif (!$category_id) {
        $_SESSION['edit-post'] = "Couldn't update the post. Invalid form data.";
    } elseif (!$body) {
        $_SESSION['edit-post'] = "Couldn't update the post. Invalid form data.";
        ...
        if ($thumbnail['name']) {
            ...
            if (in_array($extension, $allowed_files)) {
                // make sure profile is not too large (2mb+)
                if ($thumbnail['size'] < 2000000) {
                    // upload avatar
                    move_uploaded_file($thumbnail_tmp_name, $thumbnail_destination_path);
                } else {
                    $_SESSION['edit-post'] = "Couldn't update the post. Thumbnail size too big. Should be less than 2 mb";
                }
            } else {
